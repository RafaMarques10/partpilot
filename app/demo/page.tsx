...
const flow = [
  {
    title: "Escolher peca",
    text: "O aluno parte de uma familia parametrica, como flange, placa ou suporte.",
    icon: Cuboid,
  },
  {
    title: "Inserir dimensoes",
    text: "Diametros, espessura, PCD, numero de furos, material e sobremetal.",
    icon: Ruler,
  },
  {
    title: "Gerar outputs",
    text: "A app converte parametros em CAD, script, CNC e setup sheet.",
    icon: Factory,
  },
];
    ...
    <main className="min-h-screen px-5 py-6 text-graphite md:px-8">
      <section className="mx-auto flex max-w-7xl flex-col gap-6">
        <header className="rounded-lg border border-slate-200 bg-white/94 p-5 shadow-panel md:p-7">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              <Link
                href="/"
                className="inline-flex items-center gap-2 text-sm font-bold text-slate-600 hover:text-coolant"
              >
                <ArrowLeft size={16} />
                Dashboard
              </Link>
              ...
              <h1 className="mt-4 max-w-4xl text-3xl font-bold md:text-5xl">
                PartPilot mostra a ligacao entre CAD, CAM e fabrico sem sair do
                browser
              </h1>
              <p className="mt-4 max-w-3xl text-base leading-7 text-slate-600">
                Para estudantes de CAD/CNC, o maior salto nao e desenhar um
                circulo: e perceber como uma geometria parametrica vira uma
                sequencia de modelacao, um script, uma estrategia CNC e uma
                folha de setup coerente.
              </p>
            </div>
            <Link
              href="/novo-projeto"
              className="inline-flex h-11 items-center justify-center gap-2 rounded-md bg-graphite px-4 text-sm font-bold text-white hover:bg-slate-700"
            >
              Abrir flange
              <ArrowRight size={17} />
            </Link>
        ...
        <section className="grid gap-5 lg:grid-cols-2">
          <Panel
            title="Exemplo: flange parametrica"
            icon={<Cuboid size={20} />}
          >
            ...
            <div className="mt-4 grid gap-2 text-sm sm:grid-cols-2">
              ...
              <Spec
                label="Furacao"
                value={`${params.boltCount}x OD${params.boltHoleDiameter} em PCD ${params.boltCircleDiameter}`}
              />
            </div>
          </Panel>
          ...
          <Panel title="Outputs gerados" icon={<FileSpreadsheet size={20} />}>
            <div className="grid gap-3">
              <Output
                icon={<Cuboid size={18} />}
                title="Preview 2D e isometrica"
                text="A geometria reage a diametros, espessura, PCD e numero de furos."
              />
              <Output
                icon={<ClipboardList size={18} />}
                title="Passos Inventor"
                text={`${cadSteps.length} passos de modelacao com sketch, extrusao, furo central e padrao circular.`}
              />
              <Output
                icon={<Code2 size={18} />}
                title="Script Inventor"
                text="Codigo iLogic/VBA base com parametros claros e comentarios por bloco."
              />
              <Output
                icon={<Factory size={18} />}
                title="Plano CNC"
                text={`${cncPlan.length} operacoes com setup, ferramenta, estrategia, notas e tempo estimado.`}
              />
              <Output
                icon={<FileSpreadsheet size={18} />}
                title="Setup sheet"
                text="Resumo pronto para exportar em PDF pelo browser."
              />
            </div>
          </Panel>
        </section>
        ...
        <section className="grid gap-5 lg:grid-cols-[0.72fr_1.28fr]">
          ...
          <Panel
            title="Arquitetura preparada para varias pecas"
            icon={<Ruler size={20} />}
          >
            <div className="grid gap-3 md:grid-cols-3">
            ...
            <p className="mt-4 text-sm leading-6 text-slate-600">
              A segunda peca usa o mesmo pipeline da flange: parametros,
              validacao, previews SVG, script Inventor, operacoes CNC e setup
              sheet. Isto mostra que o PartPilot e uma plataforma expansivel,
              nao uma demo unica.
            </p>
          </Panel>
        </section>
              ...
              <tbody>
                {setupRows.slice(0, 4).map(([label, value]) => (
                  <tr key={label} className="border-b border-slate-200">
                    <th className="py-3 pr-3 text-left font-semibold text-slate-500">
                      {label}
                    </th>
