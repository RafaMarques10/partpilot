...
const partTypes = [
  {
    ...
    status: "Completa",
    icon: Ruler,
    tone: "bg-coolant/10 text-coolant",
  },
  {
    name: "Placa furada",
    status: "Completa",
        ...
        <header className="overflow-hidden rounded-lg border border-slate-200 bg-white/92 shadow-panel">
          <div className="grid gap-0 lg:grid-cols-[1.08fr_0.92fr]">
            <div className="p-5 md:p-7">
              ...
              <h1 className="mt-4 text-3xl font-bold text-graphite md:text-5xl">
                Assistente CAD/CNC para aulas tecnicas
              </h1>
              <p className="mt-4 max-w-3xl text-base leading-7 text-slate-600">
                Uma ferramenta gratuita para transformar parametros de uma peca
                em visualizacao, roteiro de modelacao no Inventor, script base,
                plano CNC e setup sheet exportavel.
              </p>
            ...
            <div className="border-t border-slate-200 bg-slate-50 p-5 lg:border-l lg:border-t-0">
              <div className="h-full min-h-[320px] rounded-md border border-slate-200 bg-white">
                {latestProject?.partType === "drilled-plate" ? (
                  <DrilledPlateIsoPreview
                    params={latestProject.parameters as DrilledPlateParameters}
                  />
                ) : (
                  <FlangeIsoPreview
                    params={
                      (latestProject?.parameters as
                        | FlangeParameters
                        | undefined) ?? defaultFlangeParameters
                    }
                  />
                )}
              </div>
        ...
        <div className="grid gap-5 lg:grid-cols-[1.05fr_0.95fr]">
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Guardados no browser para uma demo rapida em sala.
                </p>
              </div>
            ...
            <div className="mt-5 divide-y divide-slate-200">
              {projects.length === 0 ? (
                <div className="rounded-md border border-dashed border-slate-300 bg-slate-50 p-6 text-sm leading-6 text-slate-600">
                  Ainda nao ha projetos. Cria uma flange e o PartPilot guarda o
                  rascunho localmente.
                </div>
              ) : (
                  ...
                  >
                    <div>
                      ...
                      <div className="text-sm text-slate-500">
                        {formatProjectSummary(project)} - atualizado{" "}
          ...
          <section className="rounded-lg border border-slate-200 bg-white/94 p-5 shadow-panel">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Do parametro ao documento de fabrico, sem APIs pagas.
            </p>
            <div className="mt-5 grid gap-3">
              {outputs.map((output, index) => {
                ...
                return (
                  ...
                  >
                    <span className="flex h-8 w-8 items-center justify-center rounded-md bg-coolant/10 text-sm font-bold text-coolant">
                      {index + 1}
                    </span>
        ...
        <section className="rounded-lg border border-slate-200 bg-white/94 p-5 shadow-panel">
          <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                A arquitetura ja separa tipos de peca, parametros e geradores.
              </p>
            </div>
            <Link
              href="/novo-projeto"
              className="inline-flex items-center gap-2 text-sm font-bold text-graphite hover:text-coolant"
            >
              Abrir configurador
              <ArrowRight size={16} />
            </Link>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {partTypes.map((part) => {
              ...
              return (
                ...
                >
                  <span
                    className={`inline-flex h-10 w-10 items-center justify-center rounded-md ${part.tone}`}
                  >
                    <Icon size={21} />
                  </span>
