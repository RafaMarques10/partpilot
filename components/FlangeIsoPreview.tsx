...
export function FlangeIsoPreview({ params }: FlangeIsoPreviewProps) {
  ...
  const pcdRatio =
    params.boltCircleDiameter / Math.max(params.outerDiameter, 1);
  ...
  const boltAngles = Array.from(
    { length: params.boltCount },
    (_, index) => (Math.PI * 2 * index) / params.boltCount,
  );
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Pre-visualizacao isometrica da flange"
    >
      <defs>
      ...
      <path
        d="M 46 308 H 514 M 82 340 H 478 M 122 282 H 438"
        stroke="#d7e1e6"
        strokeWidth="1"
        strokeDasharray="8 8"
      />
      ...
      <g fill="#334155" fontSize="13">
        ...
        <text x="360" y="338" textAnchor="middle">
          {params.boltCount} furos OD{params.boltHoleDiameter} em PCD{" "}
