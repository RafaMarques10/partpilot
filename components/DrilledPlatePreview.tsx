...
export function DrilledPlatePreview({ params }: DrilledPlatePreviewProps) {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="h-full w-full" role="img" aria-label="Pre-visualizacao 2D da placa furada">
      <defs>
        ...
        <filter id="plate-shadow" x="-20%" y="-20%" width="140%" height="140%">
          <feDropShadow dx="0" dy="12" stdDeviation="10" floodColor="#14202b" floodOpacity="0.14" />
        </filter>
      </defs>
      <rect width={width} height={height} rx="6" fill="#f7fafb" />
      <path d={`M ${x0 - 18} ${height / 2} H ${x0 + plateW + 18} M ${width / 2} ${y0 - 18} V ${y0 + plateH + 18}`} stroke="#d4dee3" strokeWidth="1" strokeDasharray="5 7" />
      <rect x={x0} y={y0} width={plateW} height={plateH} rx="7" fill="url(#plate-top)" stroke="#475569" strokeWidth="2" filter="url(#plate-shadow)" />
      {holes.map((hole, index) => (
        <circle key={`${hole.x}-${hole.y}-${index}`} cx={hole.x} cy={hole.y} r={holeR} fill="#f7fafb" stroke="#273545" strokeWidth="1.8" />
      ))}
      <line x1={x0} y1={y0 + plateH + 26} x2={x0 + plateW} y2={y0 + plateH + 26} stroke="#d6502a" strokeWidth="2" />
      <text x={width / 2} y={y0 + plateH + 49} textAnchor="middle" fill="#334155" fontSize="15">
        {params.length} x {params.width} mm
      </text>
      <text x={width / 2} y={y0 - 18} textAnchor="middle" fill="#334155" fontSize="13">
